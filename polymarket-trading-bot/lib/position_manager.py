...
class Position:
    ...
    def take_profit_price(self) -> float:
        """Target price for take profit (entry * (1 + pct))."""
        return self.entry_price * (1 + self.take_profit_delta)

    @property
    def stop_loss_price(self) -> float:
        """Target price for stop loss (entry * (1 - pct))."""
        return self.entry_price * (1 - self.stop_loss_delta)
