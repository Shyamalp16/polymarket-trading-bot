...
class GammaClient(ThreadLocalSessionMixin):
    ...
    # Supported coins and their slug prefixes (by duration)
    COIN_SLUGS = {
        5: {
            "BTC": "btc-updown-5m",
            "ETH": "eth-updown-5m",
    ...
    def get_current_market(self, coin: str, duration: int = 15) -> Optional[Dict[str, Any]]:
        """
        Get the current active market for a coin at a given duration.
        ...
        """
        coin = coin.upper()
        slugs = self.COIN_SLUGS.get(duration)
        if not slugs:
            raise ValueError(f"Unsupported duration: {duration}m. Use: {list(self.COIN_SLUGS.keys())}")
        if coin not in slugs:
            raise ValueError(f"Unsupported coin: {coin}. Use: {list(slugs.keys())}")

        prefix = slugs[coin]
        window_seconds = duration * 60
        now = datetime.now(timezone.utc)

        # Round to current N-minute window
        minute = (now.minute // duration) * duration
    ...
    def get_current_15m_market(self, coin: str) -> Optional[Dict[str, Any]]:
        ...
        """
        return self.get_current_market(coin, duration=15)
    ...
    def get_current_5m_market(self, coin: str) -> Optional[Dict[str, Any]]:
        ...
        """
        return self.get_current_market(coin, duration=5)

    def get_next_market(self, coin: str, duration: int = 15) -> Optional[Dict[str, Any]]:
        """
        ...
        slugs = self.COIN_SLUGS.get(duration)
        if not slugs:
            raise ValueError(f"Unsupported duration: {duration}m")
        if coin not in slugs:
        ...
        # Calculate next N-minute window
        minute = ((now.minute // duration) + 1) * duration
        if minute >= 60:
    ...
    def get_market_info(self, coin: str, duration: int = 15) -> Optional[Dict[str, Any]]:
        """
        Get comprehensive market info for the current active market.
        ...
        """
        market = self.get_current_market(coin, duration=duration)
        if not market:
            return None
