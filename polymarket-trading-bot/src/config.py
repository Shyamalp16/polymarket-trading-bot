...
class ClobConfig:
    ...
    signature_type must match how your Polymarket account was created:
        0 = EOA (MetaMask, hardware wallet — private key trades directly)
        1 = Email / Magic wallet proxy (funder = your proxy address)
        2 = Browser wallet proxy (funder = your proxy address, Gnosis Safe)

    Using the WRONG signature_type causes "not enough balance / allowance"
    errors because the CLOB looks up balance on the wrong address.
    """
    host: str = "https://clob.polymarket.com"
    chain_id: int = 137
    signature_type: int = 2  # 0=EOA, 1=Email/Magic proxy, 2=Browser wallet proxy
